...
class ProfesseurCoursTabView extends StatefulWidget {
  final GlobalKey<ScaffoldMessengerState> scaffoldMessengerKey;

  ProfesseurCoursTabView({required this.scaffoldMessengerKey});

  @override
  _CoursTabViewState createState() => _CoursTabViewState();
}

class _CoursTabViewState extends State<ProfesseurCoursTabView> with SingleTickerProviderStateMixin {
      ...
      children: [
        ...
        Expanded(
          child: TabBarView(
            controller: _tabController,
            children: [
              CoursList(endpoint: 'professeur/cours', scaffoldMessengerKey: widget.scaffoldMessengerKey),
              CoursList(endpoint: 'professeur/cours-a-venir', scaffoldMessengerKey: widget.scaffoldMessengerKey),
              CoursList(endpoint: 'professeur/cours-termines-ou-annules', scaffoldMessengerKey: widget.scaffoldMessengerKey),
            ],
          ),
        ),
      ],
...
class CourCard extends StatelessWidget {
  ...
  void _montrerDialogPresence(BuildContext context, Cour cour) {
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: const Text('Présence du professeur'),
          content: const Text('Le professeur était-il présent pour ce cours ?'),
          actions: <Widget>[
            TextButton(
  ...
  void _mettreAJourPresence(BuildContext context, int coursId, bool present) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        scaffoldMessengerKey.currentState?.showSnackBar(
          const SnackBar(content: Text('Erreur de mise à jour de la présence.')),
        );
      }
    } catch (e) {
      scaffoldMessengerKey.currentState?.showSnackBar(
  ...
  void _mettreAJourStatut(BuildContext context, int coursId, String statut) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        scaffoldMessengerKey.currentState?.showSnackBar(
          const SnackBar(content: Text('Erreur de mise à jour du statut.')),
        );
      }
    } catch (e) {
      scaffoldMessengerKey.currentState?.showSnackBar(
  ...
  void _mettreAJourDispense(BuildContext context, int coursId, bool dispense) async {
    try {
      ...
      if (response.statusCode == 200) {
        scaffoldMessengerKey.currentState?.showSnackBar(
          const SnackBar(content: Text('Dispense mise à jour avec succès.')),
        );
      } else {
        scaffoldMessengerKey.currentState?.showSnackBar(
          const SnackBar(content: Text('Erreur de mise à jour de la dispense.')),
        );
      }
    } catch (e) {
      scaffoldMessengerKey.currentState?.showSnackBar(
