...
def index(request):
    ...
    revenu_total_mensuel = Cours.objects.filter(date__month=mois_actuel).aggregate(Sum('montant'))['montant__sum'] or 0
    revenu_total_annuel = Cours.objects.filter(date__year=annee_actuelle).aggregate(Sum('montant'))['montant__sum'] or 0

    # Statistiques des cours pour chaque type d'utilisateur
    nombre_cours_parents = Cours.objects.filter(eleve__isnull=False).count()
    nombre_cours_professeurs = Cours.objects.filter(professeur__isnull=False).count()
    nombre_cours_eleves = Cours.objects.filter(eleve__isnull=False).count()
    ...
    context = {
        ...
        'nombre_cours_parents': nombre_cours_parents,
        'nombre_cours_professeurs': nombre_cours_professeurs,
        'nombre_cours_eleves': nombre_cours_eleves,
